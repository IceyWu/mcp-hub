...
function createServer(): McpServer {
  ...
  server.registerTool(
    ...
    async (input) =>
      result(
        await apiRequest<PageResult<Essay>>("/api/v1/essays", { query: input }),
      ),
  );
  ...
  server.registerTool(
    "create_excerpt",
    {
      title: "新增书摘",
      description:
        "向 WordsEssence 新增一条书摘。这是写操作，调用前应确认正文及可选的作者、书名和标题。",
      inputSchema: essayCreateSchema,
    },
    async (input) =>
      result(
        await apiRequest<Essay>("/api/v1/essays", {
          method: "POST",
          body: input,
        }),
      ),
  ...
  server.registerTool(
    "update_excerpt",
    {
      title: "更新书摘",
      description:
        "按 ID 更新书摘的指定字段。这是写操作，未提供的字段保持不变。",
      inputSchema: { id: z.number().int().positive(), ...essayUpdateSchema },
    },
    async ({ id, ...body }) => {
      if (Object.keys(body).length === 0)
        throw new Error("至少提供一个要更新的书摘字段");
      return result(
        await apiRequest<Essay>(`/api/v1/essays/${id}`, {
          method: "PUT",
          body,
        }),
      );
  ...
  server.registerTool(
    "delete_excerpt",
    {
      title: "删除书摘",
      description:
        "按 ID 软删除书摘。这是删除操作，调用前必须向用户确认目标 ID。",
      inputSchema: { id: z.number().int().positive() },
    },
    async ({ id }) => {
...
async function startHttp(port: number): Promise<void> {
  ...
  app.listen(port, "127.0.0.1", () =>
    console.error(`WordsEssence MCP listening on http://127.0.0.1:${port}/`),
  );
}
